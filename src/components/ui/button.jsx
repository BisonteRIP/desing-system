...
function Button({ size = "default", variant = "default", children, ...props }) {
  const variantCheckClass =
    buttonVariantStyle[variant] || buttonVariantStyle.default;
  const sizeCheckClass = buttonSizeStyle[size] || buttonSizeStyle.default;
  const finalClass =
    `${buttonBaseStyle} ${variantCheckClass} ${sizeCheckClass}`.trim();
  return (
